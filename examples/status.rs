...
use miners::protocol::netty::status::clientbound::Ping0 as Pong;
...
use miners::{net::conn::Connection, version::ProtocolVersion}; // should probably be renamed to pong
...
async fn main() {
    ...
    conn.write_half
        .write_packet(
            version,
            Handshake {
                // it would be nice if we didn't have to access the write/read halves as fields
        ...
        .await
        .unwrap(); // We should also add an abstraction (in craftrs.unwrap()) so you don't have to pass around encoders
                   // initiate server list ping
    conn.write_half
        .write_packet(version, Request {}, &mut encoder)
        .await
        .unwrap(); // Request0 should probably be a marker struct instead so you don't need {}
    ...
    // decode response
    let resp = Response::decode(&mut Cursor::new(data))
        .unwrap()
        .data
        .to_owned(); // We should add a method so we don't need to construct the Cursor ourselves.
    println!("{ADDRESS}:\n\tRESPONSE: {resp}");
    // ping
    let time = std::time::Instant::now();
    let payload = chrono::Utc::now().timestamp();
    conn.write_half
        .write_packet(version, Ping { time: payload }, &mut encoder)
        .await
        .unwrap();
