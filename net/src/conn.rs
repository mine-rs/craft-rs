...
use miners::{protocol::Packet, net::encoding::Encoder};
...
pub struct Connection {
    inner: miners::net::conn::Connection<BufReader<Compat<OwnedReadHalf>>, BufWriter<Compat<OwnedWriteHalf>>>
}
...
impl ReadHalf {
    pub(super) fn new(inner: miners::net::conn::ReadHalf<BufReader<Compat<OwnedReadHalf>>>) -> Self {
        Self {
            inner
        }
    }
...
impl WriteHalf {
    pub(super) fn new(inner: miners::net::conn::WriteHalf<BufWriter<Compat<OwnedWriteHalf>>>) -> Self {
        Self {
            encoder: Encoder::new(),
            inner
        }
    ...
    pub async fn write_packet(&mut self, version: i32, packet: impl Packet) -> miners::encoding::encode::Result<()> {
        self.inner.write_packet(version, packet, &mut self.encoder).await
    }
}
